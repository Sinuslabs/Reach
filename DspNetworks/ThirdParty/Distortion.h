...
#include "src/Distortion.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct DistortionMetaData {
		SN_NODE_ID("Distortion");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using Distortion = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _Distortion, DistortionMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros
