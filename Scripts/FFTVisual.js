namespace FFTVisual {
	...
	const var DB = Synth.getDisplayBufferSource("FFT Analyser");
	...
	// Create the path in the background
	inline function getPath(BGT) {

		path = buffer.createPath(Panel1.getLocalBounds(0), // area
			[0, BUFFER_LENGTH, 0, 1.0], // [x_start, x_end, y_min, y_max]
			0.0 // start value (the initial value at position 0)
		);
		updateFFTHistory(path);
		Panel1.repaint();
		return;
	}
	...
	Panel1.setTimerCallback(function () {
		BGT.callOnBackgroundThread(getPath);
	});
	...
	inline function updateFFTHistory(fftPath) {
		if (typeof (fftPath) == "object" && isDefined(fftPath)) {
	        local newPath = fftPath;
